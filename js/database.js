...
export function initializeDatabase(db) {
  ...
  async function addToDatabase(name) {
    try {
      // Додавання документу з текстом "привіт" та значенням name у вашу колекцію "greetings"
      await addDoc(collection(db, "Users"), {
        message: "привіт",
        name: name // Додаємо ім'я до документу
      });
  ...
  window.onload = function() {
    // перевіряємо чи існує змінна name
    if (typeof name !== 'undefined') {
      addToDatabase(name); // якщо існує, передаємо її значення у функцію addToDatabase
    } else {
      console.error("Змінна name не визначена");
    }
  };
}
